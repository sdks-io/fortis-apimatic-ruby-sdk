
# V1 Signatures Request

*This model accepts additional fields of type Object.*

## Structure

`V1SignaturesRequest`

## Fields

| Name | Type | Tags | Description |
|  --- | --- | --- | --- |
| `signature` | `String` | Required | Signature |
| `resource` | [`Resource`](../../doc/models/resource.md) | Required | - |
| `resource_id` | `String` | Required | Resource ID<br><br>**Constraints**: *Pattern*: `^(([0-9a-fA-F\-]{24,36})\|(([0-9a-fA-F]{8})-(([0-9a-fA-F]{4}\-){3})([0-9a-fA-F]{12})))$` |
| `additional_properties` | `Hash[String, Object]` | Optional | - |

## Example (as JSON)

```json
{
  "signature": "",
  "resource": "AccountVault",
  "resource_id": "11e95f8ec39de8fbdb0a4f1a",
  "exampleAdditionalProperty": {
    "key1": "val1",
    "key2": "val2"
  }
}
```

