...
module FortisApi
  ...
  class ResponseTokensCollection < BaseModel
    SKIP = Object.new
    ...
    def initialize(type = Type94Enum::TOKENSCOLLECTION, list = SKIP,
    ...
    def self.from_hash(hash)
      ...
      type = hash['type'] ||= Type94Enum::TOKENSCOLLECTION
      # Parameter is an array, so we need to iterate through it
      list = nil
      unless hash['list'].nil?
        list = []
        hash['list'].each do |structure|
          list << (List16.from_hash(structure) if structure)
        end
      end
      ...
      ResponseTokensCollection.new(type,
                                   list,
                                   links,
                                   pagination,
                                   sort,
