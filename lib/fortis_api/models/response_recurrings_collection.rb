...
module FortisApi
  ...
  class ResponseRecurringsCollection < BaseModel
    SKIP = Object.new
    ...
    def initialize(type = Type68Enum::RECURRINGSCOLLECTION, list = SKIP,
    ...
    def self.from_hash(hash)
      ...
      type = hash['type'] ||= Type68Enum::RECURRINGSCOLLECTION
      # Parameter is an array, so we need to iterate through it
      list = nil
      unless hash['list'].nil?
        list = []
        hash['list'].each do |structure|
          list << (List11.from_hash(structure) if structure)
        end
      end
      ...
      ResponseRecurringsCollection.new(type,
                                       list,
                                       links,
                                       pagination,
                                       sort,
