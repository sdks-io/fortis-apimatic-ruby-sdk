...
module FortisApi
  ...
  class ResponseTicketsCollection < BaseModel
    SKIP = Object.new
    ...
    def initialize(type = Type88Enum::TICKETSCOLLECTION, list = SKIP,
    ...
    def self.from_hash(hash)
      ...
      type = hash['type'] ||= Type88Enum::TICKETSCOLLECTION
      # Parameter is an array, so we need to iterate through it
      list = nil
      unless hash['list'].nil?
        list = []
        hash['list'].each do |structure|
          list << (List15.from_hash(structure) if structure)
        end
      end
      ...
      ResponseTicketsCollection.new(type,
                                    list,
                                    links,
                                    pagination,
                                    sort,
