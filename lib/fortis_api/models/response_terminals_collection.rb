...
module FortisApi
  ...
  class ResponseTerminalsCollection < BaseModel
    SKIP = Object.new
    ...
    def initialize(type = Type83Enum::TERMINALSCOLLECTION, list = SKIP,
    ...
    def self.from_hash(hash)
      ...
      type = hash['type'] ||= Type83Enum::TERMINALSCOLLECTION
      # Parameter is an array, so we need to iterate through it
      list = nil
      unless hash['list'].nil?
        list = []
        hash['list'].each do |structure|
          list << (List14.from_hash(structure) if structure)
        end
      end
      ...
      ResponseTerminalsCollection.new(type,
                                      list,
                                      links,
                                      pagination,
                                      sort,
