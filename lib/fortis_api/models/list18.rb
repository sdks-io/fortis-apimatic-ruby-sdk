...
module FortisApi
  ...
  class List18 < BaseModel
    ...
    # TagTransaction Information on `expand`
    # @return [DeclinedRecurringNotification1]
    attr_accessor :declined_recurring_notification

    # TagTransaction Information on `expand`
    # @return [PaymentRecurringNotification1]
    ...
    # [object Object]
    # @return [RejectedTransactionAchRetry]
    ...
    def initialize(additional_amounts: SKIP, billing_address: SKIP,
                   ...
                   customer_id: SKIP, description: SKIP, iias_ind: SKIP,
                   ...
                   is_completable: SKIP, is_settled: SKIP, created_user: SKIP,
                   location: SKIP, contact: SKIP, changelogs: SKIP,
                   product_transaction: SKIP, all_tags: SKIP,
                   tag_transactions: SKIP,
                   ...
                   signature: SKIP, reason_code: SKIP, type: SKIP, status: SKIP,
                   transaction_batch: SKIP, transaction_splits: SKIP,
                   postback_logs: SKIP, currency_type: SKIP,
                   transaction_references: SKIP,
    ...
    def self.from_hash(hash)
      ...
      recurring = Recurring.from_hash(hash['recurring']) if hash['recurring']
      recurring_number =
      ...
      quick_invoice = QuickInvoice1.from_hash(hash['quick_invoice']) if hash['quick_invoice']
      # Parameter is an array, so we need to iterate through it
      log_emails = nil
      ...
      if hash['declined_recurring_notification']
        declined_recurring_notification = DeclinedRecurringNotification1.from_hash(hash['declined_recurring_notification'])
      end
      if hash['payment_recurring_notification']
        payment_recurring_notification = PaymentRecurringNotification1.from_hash(hash['payment_recurring_notification'])
      end
      ...
      hosted_payment_page = HostedPaymentPage1.from_hash(hash['hosted_payment_page']) if
        hash['hosted_payment_page']
      transaction_level3 = Data29.from_hash(hash['transaction_level3']) if
      ...
      surcharge_transaction = SurchargeTransaction1.from_hash(hash['surcharge_transaction']) if
      ...
      type = Type7.from_hash(hash['type']) if hash['type']
      status = Status7.from_hash(hash['status']) if hash['status']
      transaction_batch = TransactionBatch1.from_hash(hash['transaction_batch']) if
      ...
      currency_type = CurrencyType1.from_hash(hash['currency_type']) if hash['currency_type']
      # Parameter is an array, so we need to iterate through it
      ...
      if hash['return_fee_transaction_ach_retry']
        return_fee_transaction_ach_retry = RejectedTransactionAchRetry.from_hash(hash['return_fee_transaction_ach_retry'])
      end
      if hash['retry_transaction_ach_retry']
        retry_transaction_ach_retry = RejectedTransactionAchRetry.from_hash(hash['retry_transaction_ach_retry'])
      end
      is_retriable = hash.key?('is_retriable') ? hash['is_retriable'] : SKIP
      saved_account = SavedAccount1.from_hash(hash['saved_account']) if hash['saved_account']
      # Parameter is an array, so we need to iterate through it
      balances = nil
      ...
      List18.new(additional_amounts: additional_amounts,
    ...
    def inspect
      ...
      " #{@balances.inspect}, additional_properties: #{@additional_properties}>"
    end
  end
end
