...
module FortisApi
  ...
  class ResponseSignaturesCollection < BaseModel
    SKIP = Object.new
    ...
    def initialize(type = Type73Enum::SIGNATURESCOLLECTION, list = SKIP,
    ...
    def self.from_hash(hash)
      ...
      type = hash['type'] ||= Type73Enum::SIGNATURESCOLLECTION
      # Parameter is an array, so we need to iterate through it
      list = nil
      unless hash['list'].nil?
        list = []
        hash['list'].each do |structure|
          list << (List12.from_hash(structure) if structure)
        end
      end
      ...
      ResponseSignaturesCollection.new(type,
                                       list,
                                       links,
                                       pagination,
                                       sort,
